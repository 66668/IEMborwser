 ...
 *
 */
public class SquareSurfacePreview extends SurfaceView {

    public static final String TAG = "SquareSurfacePreview";
    ...
    private static final double QUEUE_RATIO = 9.0 / 16.0;
    ...
    public SquareSurfacePreview(Context context) {
        super(context);
        init(context);
    }

    public SquareSurfacePreview(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    public SquareSurfacePreview(Context context, AttributeSet attrs, int defStyle) {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        Log.d(TAG, "SquareSurfacePreview--width=" + width+"--height="+height);
        if (width > height * ASPECT_RATIO) {
            //当出现特殊尺寸pad时，手动修改数值就可以调整，但是还是要调整拍照后的预览尺寸，否则图片拉伸，影响效果
            width = (int) (height * ASPECT_RATIO + 0.5);//QUEUE_RATIO
            Log.d(TAG, "SquareSurfacePreview--width=" + width);
        }
        else {
            height = (int) (width / ASPECT_RATIO + 0.5);
            Log.d(TAG, "SquareSurfacePreview--height=" + height);
        }
    ...
    public void setCamera(Camera camera) {
        ...
        if (camera != null) {
            ...
            mIsZoomSupported = params.isZoomSupported();//是否支持缩放
            if (mIsZoomSupported) {
